...
def parse_document(file_path, llama_parse_api_key):
    ...
    try:
        ...
        documents = []
        """In LangChain, almost every downstream component—splitters, retrievers, vector stores, chains—expects inputs in the form of Document objects rather than raw strings or miscellaneous dicts. Converting your parsed output into langchain_core.documents.Document instances gives you several key benefits:"""
        # Check if result is a list
        if isinstance(result, list):  # Prepare a list to collect uniform LangChain Document objects
            for item in result:
...
def create_semantic_chunks(documents):
    ...
    try:
        ...
        semantic_chunker = SemanticChunker(
            embeddings,
            breakpoint_threshold_type="percentile",  # use percentile for semantic chunking
            breakpoint_threshold_amount=90,
            min_chunk_size=50,      
            buffer_size=0,                           # no overlap between chunks
